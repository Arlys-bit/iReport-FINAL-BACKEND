...
import { liveIncidentsApi } from '@/utils/api';
...
export const [LiveReportsProvider, useLiveReports] = createContextHook(() => {
  ...
  const createIncidentMutation = useMutation({
    mutationFn: async (data: {
      ...
      try {
        // Try API first
        const result: any = await liveIncidentsApi.createIncident(data);
        queryClient.invalidateQueries({ queryKey: ['liveIncidents'] });
        return result;
      } catch (apiError) {
        logger.warn('API create live incident failed, using local storage:', apiError);
  ...
  const respondToIncidentMutation = useMutation({
    mutationFn: async ({
      ...
      try {
        // Try API first
        const result: any = await liveIncidentsApi.respondToIncident(incidentId, {
          userName,
        });
        queryClient.invalidateQueries({ queryKey: ['liveIncidents'] });
        return result;
      } catch (apiError) {
        logger.warn('API respond to incident failed, using local storage:', apiError);
